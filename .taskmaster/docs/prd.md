...
**버전:** 1.4 (2026-05-27 잔여 미해결 질문 3건 확정)
**최종 수정일:** 2026-05-27
**상태:** Draft → **Approved** (모든 핵심 의사결정 완료)
...
- 본업과 병행 가능한 활동 (월 4~6편 발행, 인센티브 30만원)
...
**So that I can** 본업과 병행하며 월 4~6편을 안정적으로 발행할 수 있다.
...
```
┌───────────────────────────────────────────────────────────────────┐
│ Layer 1: Orchestration  (LangGraph Lite)                         │
│   Router → [Prediction Agent | Generation Agent]                 │
│   캐시 우선 확인, 미스 시에만 LLM 호출                              │
└─────────────────────────────┬─────────────────────────────────────┘
                              ↓
┌───────────────────────────────────────────────────────────────────┐
│ Layer 2: Core Processing & Data                                  │
│  • Quantized LLM (Solar-10.7B / Llama-3.1-8B/14B)                │
│  • PGVector / Chroma (RAG)                                       │
│  • Lite Knowledge Graph (Neo4j / NetworkX)                       │
└─────────────────────────────┬─────────────────────────────────────┘
                              ↓
┌───────────────────────────────────────────────────────────────────┐
│ Layer 3: Infrastructure & Quality Control (v1.2 Hybrid)          │
│  • Anthropic Claude Batch API (비동기·50% 할인)                  │
│  • Together AI Solar Mini API (실시간·한국어)                    │
│  • Docker / docker-compose (백엔드·Redis만)                       │
...
**핵심 컴포넌트:**
1. **LangGraph Lite Router** — 질의 분류·라우팅, LLM 호출 최소화
2. **Quantized LLM Server** (vLLM 또는 TGI) — 자체 호스팅 추론
3. **PGVector RAG** — 20년치 기사 + 공공 데이터 임베딩
4. **Lite Knowledge Graph** — 장소·인물·사건 엔티티 관계
5. **Redis Cache** — 응답·세션·속도 제한
6. **Batch Scheduler** — 주간/일일 배치
7. **HITL Workflow** — 편집자 검토 단계 강제
...
- [ ] Task 1.7: 시민기자 모집 공고·선발(12명, 읍·면 균형) — Medium (12h, 사업 운영)
...
**Sub-Phase 2D: 시민기자 교육 (6~7월)**
- [ ] Task 2.19: 6회 교육 프로그램 커리큘럼·교재 — Medium (12h, 사업 운영)
...
**예산 정합성 (총 50,000,000원, v1.3 재배분):**

| 항목 | v1.0 (원안) | v1.3 (재배분) | Δ | 비율 | 매핑된 PRD 영역 |
|------|------|------|---|------|--------------|
| AI Core Engine 개발 | 15,500,000원 | **17,500,000원** | **+2,000,000** | 35% | REQ-AI-001~003, REQ-INFRA-001 (Hybrid Router·프롬프트·캐시) |
| 플랫폼 개발 & Hybrid | 12,500,000원 | 12,500,000원 | 0 | 25% | REQ-PLATFORM-001~003 |
| Citizen Co-Pilot & 교육 | 7,000,000원 | 7,000,000원 | 0 | 14% | REQ-CITIZEN-001 |
| 데이터 수집·인프라 | 6,000,000원 | **3,000,000원** | **−3,000,000** | 6% | REQ-DATA-001~002 (GPU PC 구매 400만원 제거 + 평가셋 강화 100만원 합산) |
| 마케팅·B2B 영업 | 6,000,000원 | **7,000,000원** | **+1,000,000** | 14% | Phase 3~4 (베타 인센티브 50명→100명) |
| PM·교육·Contingency | 3,000,000원 | 3,000,000원 | 0 | 6% | 전 Phase 관리 |
| **합계** | **50,000,000원** | **50,000,000원** | **0** | **100%** | |

**재배분 요약:**
- GPU 자체 PC 구매(약 400만원) 제거로 데이터 수집·인프라 항목이 600만 → 200만으로 줄고, 그 중 100만은 평가셋 강화로 다시 같은 항목에 환입되어 최종 300만이 됨
- AI Core Engine은 35%로 비중 확대 — 사업계획서의 "AI 기술 투자 56%" 방향성과 정합
- 마케팅 항목 +100만은 베타 NPS 신뢰도 확보 목적
...
> A: 모집 일정·자격:
> - 2026년 5월 중 모집 공고 (`taeannews.co.kr`에서 안내)
...
> 사업비 배분(31% AI Core / 25% 플랫폼 / 14% 시민기자 / 12% 데이터 / 12% 마케팅 / 6% PM·예비)이 본 태스크 구성과 정합한다.
