...
audioRouter.get("/news/:idxno", async (c) => {
  ...
  let cancelled = false;   // 클라이언트 중단 시 남은 청크 전송·캐시를 건너뛴다
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      try {
        for (let i = 0; i < promises.length; i++) {
          const b = i === 0 ? first : await promises[i];   // 순서 보장(뒤 청크는 이미 병렬 진행 중)
          if (cancelled) return;                           // 재생 중단 → 조기 종료
          if (b) { controller.enqueue(b); collected.push(b); }
        }
        controller.close();
      } catch {
        return;   // enqueue 중 스트림 취소 등 → 조용히 종료(캐시도 안 함)
      }
      // 취소 없이 전 청크가 정상일 때만 R2 캐시(부분 실패·중단분은 캐시 안 해 다음 요청에 재생성)
      if (!cancelled && collected.length === parts.length) {
