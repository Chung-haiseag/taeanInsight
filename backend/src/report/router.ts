...
reportRouter.get("/summary", async (c) => {
  ...
  const dataSources = [
    ...
    { key: "fireRisk", name: "산불위험 지수", status: "live", granularity: "태안·건조기·조건부",
      metric: "/api/conditions/fire-risk", note: "건조특보·습도·풍속·계절로 산불위험(낮음~매우높음). 봄·가을 건조기 안전. 높음+일 때만 /live 노출(평소 숨김). 단기예보·특보 재사용, 새 키 불필요" },
    { key: "farm", name: "영농 경보 (서리·한파·폭염+농사적기)", status: "live", granularity: "태안·월별·조건부",
      metric: "/api/conditions/farm", note: "서리(봄·가을 최저≤3)·한파(≤-5)·폭염(최고≥35) + 이번달 파종/수확 적기(마늘·양파·감자·고추·생강·고구마). 농업 사장님·주민. 경보/할일 있을 때만 노출. 단기예보 재사용, 새 키 불필요" },
