...
const SECTION_PLAN: Array<{ key: ReportSectionKey; title: string; prompt: string; maxTokens?: number }> = [
  ...
  },
  {
    key: "environment",
    title: "환경 모니터링",
    prompt: "오직 대기질·해양환경 자료만 사용해 이번 주 태안 해역·가로림만·신두리의 환경 상황을 수치와 함께 설명하세요. 미세먼지·초미세먼지의 일자별 변화에서 평소 대비 두드러진 값이 있으면 사실대로 짚되 원인은 단정하지 마세요. 치안·사고·정치 등 사회 사건은 절대 포함하지 마세요.",
  },
  {
    key: "realestate",
    title: "부동산·지역경제 동향",
...
7. 쉬운 용어(매우 중요): 일반 독자가 이해하기 쉬운 우리말로 쓰세요. 특히 대기질은 'PM10'·'PM2.5' 같은 코드·영문 약어를 절대 쓰지 말고 반드시 '미세먼지'(PM10)·'초미세먼지'(PM2.5)로만 표기하세요. 그 밖의 전문 약어도 풀어서 쓰세요.`;
