...
- 2026-08-17 · ①양식 수온 어장정보 승격(폴백 유지·키 미승인 확인) ②소속 검수 일괄 승격 병목 해소 — **①양식(고수온) 경보**: '※임시(표층 수온)' 주석대로 국립수산과학원 **실시간어장정보**(data.go.kr 1520635, 30분 주기, 양식장 인근 관측소 층별 수온)를 1순위로 붙이고 기존 해변 표층 평균은 폴백으로 유지. 응답에 `source/station/observedAt` 추가(실측인지 근사인지 화면이 구분). **확인 결과 아직 미승인** — `SERVICE_KEY_IS_NOT_REGISTERED_ERROR`(코드 30, 여객선의 코드 12=경로오류와 다름). 승인되면 코드 변경 없이 자동 승격. 미승인 로그는 한 줄로 축약. 엣지캐시 이름 `aqua`→`aqua_v2`. ※규격 확보 경로: data.go.kr이 WebFetch를 404로 막아 **국립수산과학원 자체 openApi 문서**에서 필드명(sta_nam_kor·obs_lay·wtr_tmp)을 얻었다. **②소속 검수(verified 248 정체)**: 원인은 도구 부재가 아니라 **처리량**이었다 — 기존 '고신뢰 일괄 승인'이 후보 1건당 HTTP 1회를 순차 전송(화면 로드분 300건만, 200건이면 200왕복·회당 1분) → 2,394건이면 로드·클릭 8회 반복. `bulkVerifyAffiliations()`로 **UPDATE 한 번**(2,394왕복→1회). **기본은 dry-run**(apply=true 명시해야 실제 기록 — 대량 변경 승인 절차를 코드로 강제), `normalizeBulkRule()`로 신뢰도 하한 0.5 클램프(저신뢰가 사실층에 올라가는 사고 방지·가드 테스트 5건), 신뢰도 히스토그램을 콘솔에 표시해 임계값을 분포 근거로 결정. UI: 신뢰도 선택 → 대상 건수 확인 → 'N건 일괄 승격'(확인창에 'AI 답변에 인용된다' 명시). 무토큰 401 확인. 백엔드 498·웹 47 통과. **실제 승격은 사용자가 콘솔에서 실행**.
