...
- 2026-07-30 · 답변 스트리밍(체감 지연↓) + 근거 단락화 + PDF 저장: /api/query?stream=1 SSE로 토큰 흘려보내 첫 글자 ~6초에 표시(완성 대기 없앰), done 이벤트에 정리본·출처·근거. 근거를 paragraphize로 문단 카드(펼침 기본), 'PDF로 저장' 버튼=근거 펼치고 window.print(전역 워터마크·페이지여백 재사용). 백엔드 stream.ts(drainSse)·sources.ts(selectSources 공유)·WorkersAiLlmClient.stream. 주의: fp8이 숫자 흘리는 오타는 교열 제거로 잔존 · backend/query/{router,stream,sources}, web query-client·api/query·paragraphize
