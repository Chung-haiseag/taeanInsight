...
- 2026-07-30 · PDF 저장 시 근거 대조 교열: 스트리밍은 fp8 단일패스라 연도 숫자를 가끔 흘림('개발은년부터'). 'PDF로 저장' 클릭 시 POST /api/query/polish {draft,evidence[]}로 [근거]와 대조해 빠진 숫자·글자 복원한 뒤 인쇄('1997년부터'·'2012년' 복원 확인). 근거에 없는 숫자는 지어내지 말라 지시, 실패 시 원본 유지. 읽기는 스트리밍(빠름)·저장물은 정확 · backend/query/router(polish), web query-client(savePdf)·api/query(polishForPdf)
