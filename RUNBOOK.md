...
- 2026-08-02 · 관리자 네비에서 지식그래프(KG) 콘솔 숨김: /admin/kg(인물·역임 수동 등록/검증)는 초기 파일럿·대부분 대체됨(군수·의원 사실은 tools/kg 시드, 인물소개는 AI 전기). ADMIN_SECTIONS에서 링크만 제거 — 라우트·데이터·관계검수 기능은 유지(필요 시 URL 직접 접근). web/src/components/admin-header.tsx
