...
- 2026-08-01 · 팟캐스트 인트로·아웃트로 음악 합성(로고송): 사용자 제공 음악(intro 12.6s·outro 14.4s, tools/podcast/assets/)을 ffmpeg로 합성. **인사말과 겹치지 않게** 크로스페이드 대신 순차 연결(concat) — [인트로 페이드인→페이드아웃]→0.35s공백→[말소리 원본]→0.35s공백→[아웃트로 페이드인→페이드아웃], +27.7s. 공용 헬퍼 tools/podcast/mix-music.mjs(assets 없으면 원본 폴백), gen-podcast·gen-briefing 업로드 직전에 연결→향후 회차 자동 적용. 기존 6편(2026-W26~W31) 백필 재합성·R2 교체(원본 로컬 백업). 라이브 확인 6편 원본+27.7s. tools/podcast/{mix-music,gen-podcast,gen-briefing,assets}
