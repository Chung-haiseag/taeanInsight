...
export function PodcastAudio() {
  ...
  // 이번 주 팟캐스트 생성(업로드)시각 — R2 객체 시각을 status API로 조회
  useEffect(() => {
    let cancelled = false;
    fetch(`${API_BASE}/api/audio/status`)
      .then((r) => (r.ok ? r.json() : null))
      .then((d: { podcastAt?: string | null } | null) => { if (!cancelled && d?.podcastAt) setGeneratedAt(d.podcastAt); })
      .catch(() => { /* 표기 생략 */ });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-foreground-muted">
            진행자 두 명이 이번 주 태안 소식을 대담으로 · AI 생성
            {generatedAt && ` · ${new Date(generatedAt).toLocaleString("ko-KR", { month: "long", day: "numeric", hour: "2-digit", minute: "2-digit", timeZone: "Asia/Seoul" })} 생성`}
          </p>
        </div>
        {state !== "playing" && (
