    ...
    <div className="space-y-5">
      ...
      <div className="text-right">
        <button type="button" onClick={() => setOpen(true)} className="text-xs font-semibold text-accent hover:underline">
          ✏️ 가게 정보 수정{brief.industry === "lodging" ? "(객실수·요금)" : brief.industry === "food" || brief.industry === "cafe" ? "(좌석수·객단가)" : brief.industry === "leisure" ? "(정원·체험료)" : brief.industry === "retail" ? "(방문객·객단가)" : brief.industry === "fishing" ? "(정원·승선료)" : ""}
        </button>
      </div>
