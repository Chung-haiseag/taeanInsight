...
const CHANGELOG: [string, string][] = [
  ["2026-08-13", "메뉴 구조 정리 — 흩어져 있던 내용을 성격별로 모음. 바다 관련(해수욕장 적합도·갯벌 물때·낚시)에 낙조·해무를 합쳐 '해변'을 바다 종합 안내로, '지금 태안' 맨 아래 묻혀 있던 부동산 실거래·수산 시세·물가·유가를 '지역경제'라는 상단 메뉴로 새로 꺼냄(데이터 출처 안내는 그 아래로). '지금 태안'은 날씨·대기질·도로 CCTV 등 실시간만 남겨 더 가벼워짐"],
  ["2026-08-13", "해변 페이지가 검색에 안 걸리던 문제 해결 — 페이지 제목·설명이 비어 있어 사이트 기본 이름으로만 나가고 사이트맵에도 빠져 있던 탓에 '태안 해수욕장 수온'·'물때'·'낙조' 같은 검색에 노출되지 않던 것을 바로잡음. 첫 화면도 옮겨간 내용을 옛 위치로 안내하던 것을 고쳐, 상단 메뉴 여섯 곳(질의응답·아카이브·인물·지금 태안·해변·지역경제)으로 바로 이어지도록 재구성"],
  ["2026-08-13", "검색 노출 전수 점검·중복 주소 정리 — 구글이 사이트를 못 읽게 막는 곳이 없는지 전부 확인(검색 로봇 차단 없음·사이트맵 38개 정상·구글 접속 정상, 차단된 것은 AI 학습용 수집기뿐). 같은 페이지가 www 붙은 주소와 http 주소로도 열려 검색 점수가 갈리던 것을 대표 주소 한 곳으로 모으도록 정리"],
  ["2026-08-12", "무료/유료 경계 정리(수익화 첫 실물) — 뉴스·아카이브·기본 실시간은 무료 유지(검색 유입·공익), 미래지향 AI 판단정보를 유료로. 주간 리포트의 심화 섹션(부동산·지역경제, 관광·기상 전망)에 미리보기+'멤버십으로 전체 보기' 적용(요약·환경·이벤트는 무료). 낙조·물때·오디오 낭독은 무료 유지(유입 미끼·고령 접근성). 모든 업셀에 전환 출처 추적 연결해 어느 프리미엄이 가입을 유발하는지 실측"],
  ["2026-08-11", "아카이브 검색 발견성(SEO) — 사이트맵을 최신 100건 → 37년치 전 기사 104,045건으로 확대, 뉴스 구조화데이터(NewsArticle) 추가, 구글 서치콘솔 등록·사이트맵 제출까지 완료(검색 유입의 근본 토대)"],
  ["2026-08-11", "기사 검색 색인 품질 개선 — 기사 제목·발췌를 서버에서 미리 렌더해 검색엔진이 실제 내용을 받도록(이전엔 '불러오는 중'만), 섹션 오분류·발행일 형식 등 메타데이터 버그 수정"],
  ["2026-08-11", "속도·안정성 — '지금 태안' 로딩 약 2.9초 → 1.8초(불필요한 순차 대기 제거), 20년 전 기사도 실시간 수집 장애와 무관하게 표시되도록 개선"],
  ["2026-08-11", "고령 독자 접근성 — 글자크기(가/가+/가++) 버튼 확대·모바일 상단 상시 노출, 안 커지던 작은 글씨 123곳을 크기 조절에 반응하도록 수정, 폼 라벨·이미지 대체텍스트 보강"],
  ["2026-08-11", "연도별 아카이브 브라우즈 신설 — 연도별로 기사를 둘러보는 페이지(/archive/연도)로 검색엔진이 전 기사에 도달하는 내부 경로 확보"],
  ["2026-08-10", "인물 검색 속도 대폭 개선 — 주요 인물 소개를 미리 생성·캐시(1,472명)하고 프로필 조회를 최적화해 새 인물 첫 클릭 로딩을 약 17초 → 1초대로 단축"],
  ["2026-08-10", "주간 팟캐스트 로고송 복구 — 인트로·아웃트로 로고송 누락을 바로잡고, 주차별 고유 주소로 바꿔 브라우저 캐시로 옛 방송이 재생되던 문제 해결"],
  ["2026-08-10", "주간 리포트 다듬기 — 대기질 표기를 'PM10/PM2.5' 대신 '미세먼지/초미세먼지' 쉬운 말로, 발행본의 특정 섹션만 다시 생성하는 편집 도구 추가"],
  ["2026-08-10", "멤버십 수요 검증 강화 — 효도 구독(자녀 결제→부모 열람) 세그먼트 추가 + 질의·기사·실시간 등 무료 페이지에 멤버십 전환 안내(어느 지점이 전환되는지 출처별 측정)"],
  ["2026-08-10", "관계도·첫 화면 다듬기 — 인물 관계도에서 의미 없는 '기타' 라벨 숨김, 신뢰 문구를 'AI가 거들고, 사람이 확인합니다'로 조정"],
  ["2026-08-09", "비로그인 랜딩 모던 프리미엄 리디자인 — 구독 전환용 첫인상 강화(히어로·멤버십 9,900원 요금·무료 vs 멤버십 비교표·주말 수요 미끼)"],
  ["2026-08-09", "인물 소개 시제 정확화 — 지난 선거·행사를 미래형·현재진행형으로 쓰던 오류 수정(오늘 날짜·최신 기사 기준 과거형, 끝난 직책은 역임형)"],
  ["2026-08-09", "인물 검색 속도 개선 — 프로필에 15분 엣지캐시(재조회·인기 인물 즉시)·주요 인물 소개 캐시 워밍으로 로딩 스피너 해소"],
  ["2026-08-09", "기사듣기 안정화 — 생성 시 WAV로 바로 업로드(ffmpeg 의존 제거) + 매일 08:00 VPS에서 MP3 자동 변환"],
  ["2026-08-09", "도로 CCTV(ITS) 요청 주기 완화(30→90분) — 개인 API 한도 초과 대응"],
  ["2026-08-08", "카카오 로그인 라이브 — 개발자앱 생성·설정, KAKAO_REST_KEY·Client Secret 반영(카카오 기본 활성화 대응), 닉네임 동의(이메일은 비즈앱 전환 후)"],
  ["2026-08-08", "도메인 이전(→axtaeannews.co.kr) 잔여 참조 전수 정리 — 로그인 리디렉트 버그·SEO(sitemap/robots)·OG·푸터·이메일 링크 교정, CORS·리디렉트 허용목록"],
  ["2026-08-08", "기사듣기(Gemini 낭독) 카페24 VPS로 일원화·안정화 — 누락된 ffmpeg 설치 복구 + ffmpeg 실패 시 WAV 폴백, 맥 의존 제거(매일 07:00 자동)"],
